...
// Ensure critical dirs exist up front
...
// In-memory training state exposed to the frontend
let trainingState = {
  is_training: false,
  current_episode: 0,
  total_episodes: 0,
...
app.post("/upload-csv", upload.single("file"), (req, res) => {
  try {
    ...
    // Move to deterministic path used by Python
    fs.renameSync(req.file.path, DATA_FILE);
    console.log("upload-csv: saved to", DATA_FILE);

    // Verify first non-empty line contains "Close"
